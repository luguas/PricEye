        ...
        {steps.map((stepName, index) => (
          <div key={index} className="flex items-center flex-1">
            <div className="flex flex-col items-center flex-1">
              ...
              <span className={`text-xs mt-2 text-center ${currentStep === index + 1 ? 'text-global-blanc' : 'text-global-inactive'}`}>
                {stepName}
              </span>
            </div>
            {index < steps.length - 1 && (
              <div className={`flex-1 h-0.5 mx-2 ${currentStep > index + 1 ? 'bg-green-600' : 'bg-global-stroke-box'}`} />
            )}
          </div>
        ))}
      </div>
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center p-4 z-50">
        <div className="bg-global-bg-box border border-global-stroke-box rounded-[14px] shadow-xl w-full max-w-5xl p-6 max-h-[90vh] flex flex-col">
            ...
            <CustomScrollbar className="flex-1 min-h-0">
              <form onSubmit={handleSubmit} className="pr-2">
                ...
                <div className="flex justify-between gap-4 pt-6 mt-4 border-t border-global-stroke-box">
                  ...
                  >
                    {isLoading ? t('common.saving') : currentStep === totalSteps ? t('propertyModal.save') : t('propertyModal.next')}
                  </button>
                </div>
              </form>
            </CustomScrollbar>
